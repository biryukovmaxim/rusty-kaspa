...
async fn run_kip10_activation_test() {
    ...
    consensus.add_utxo_valid_block_with_parents((index + 2).into(), vec![index.into()], vec![]).unwrap().await.unwrap();
    index += 1;
    assert_eq!(consensus.lkg_virtual_state.load().daa_score, 4);
    dbg!("add tx");
    let status =
        consensus.add_utxo_valid_block_with_parents((index + 2).into(), vec![index.into()], vec![spending_tx.clone()]).unwrap().await;
    ...
    // // Add one more block to reach activation score
    // consensus.add_utxo_valid_block_with_parents((index + 1).into(), vec![index.into()], vec![]).unwrap().await.unwrap();
    // index += 1;
    // assert_eq!(consensus.lkg_virtual_state.load().daa_score, 5);
    // // Test 2: Verify the same transaction is accepted after activation
    // let status = consensus.add_utxo_valid_block_with_parents((index + 2).into(), vec![index.into()], vec![spending_tx.clone()]).unwrap().await;
    // assert!(matches!(status, Ok(BlockStatus::StatusUTXOValid)));
    // assert!(consensus.lkg_virtual_state.load().accepted_tx_ids.contains(&tx_id));
}
