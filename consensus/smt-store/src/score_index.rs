...
use crate::keys::{AnyScoreIndexKey, BatchedScoreIndexKey, ScoreIndexKey, ScoreIndexKind};
...
pub type ScoreIndexChunk = (Vec<MaybeFork<Vec<Hash>>>, Option<AnyScoreIndexKey>);
...
impl DbScoreIndex {
    ...
    pub fn collect_chunk(
        ...
        while iter.valid() && entries.len() < max_entries {
            ...
            let lane_keys =
                <[Hash]>::ref_from_bytes(value_bytes).map_err(|e| StoreError::DataInconsistency(format!("score index value: {e}")))?;
            ...
            entries.push(MaybeFork::new(lane_keys.to_vec(), blue_score, block_hash));
            iter.next();
        }
...
mod tests {
    ...
    fn collect_chunk_resumes_across_calls() {
        let (_lt, store) = make_store();
        // 10 LeafUpdate entries at distinct (score, block_hash).
        for s in 100u64..110 {
            store
                .put(DirectDbWriter::new(&store.db), s, ScoreIndexKind::LeafUpdate, hash(s as u8), &[hash(0xAA), hash(s as u8)])
                .unwrap();
        }

        let reference: Vec<(u64, Hash, Vec<Hash>)> = store
            .get_all(0..=200)
            .map(|r| {
                let e = r.unwrap();
                (e.blue_score(), e.block_hash(), e.data().clone())
            })
            .collect();
        assert_eq!(reference.len(), 10);

        let mut collected: Vec<(u64, Hash, Vec<Hash>)> = Vec::new();
        ...
        loop {
            ...
            for e in &chunk {
                collected.push((e.blue_score(), e.block_hash(), e.data().clone()));
            }
            match next {
    ...
    fn collect_chunk_signals_done_when_exhausted() {
        let (_lt, store) = make_store();
        for s in 50u64..53 {
            store.put(DirectDbWriter::new(&store.db), s, ScoreIndexKind::LeafUpdate, hash(s as u8), &[hash(0x11)]).unwrap();
        }
