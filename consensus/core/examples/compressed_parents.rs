...
fn prepare_serialized() -> Vec<Vec<u8>> {
    ...
    let compact = CompressedParents::try_from(expanded).unwrap();
    let serialized_one = borsh::to_vec(&compact).unwrap();
...
fn main() {
    ...
    let _profiler = dhat::Profiler::builder().file_name("compressed.json").build();

    let start = Instant::now();

    // Looped work: deserialize each + black_box the structure
    for serialized in serialized_vec.iter() {
        let compact: CompressedParents = CompressedParents::try_from_slice(serialized).unwrap();
        black_box(&compact);
    }

    let elapsed = start.elapsed().as_millis();
    println!("Compact Time: {} ms", elapsed);

    let stats = dhat::HeapStats::get();
    println!("Compact Stats: {:?}", stats);
}
