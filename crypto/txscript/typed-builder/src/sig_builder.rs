...
// ---------------------------------------------------------------------------
// Helper: serialize via ScriptBuilder, reverse, append to buf
// ---------------------------------------------------------------------------

fn push_reversed(buf: &mut Vec<u8>, tmp: &mut ScriptBuilder) {
    let bytes = tmp.drain();
...
impl<M> TypedScriptBuilder<Bool<()>, M> {
    ...
    pub fn into_sig_builder(mut self) -> ScriptSignatureBuilder<M> {
        ...
        ScriptSignatureBuilder { redeem_script, buf: Vec::new(), _phantom: PhantomData }
    }
}
...
impl<M> ScriptSignatureBuilder<Num<M>> {
    ...
    pub fn add_i64(mut self, val: i64) -> ScriptSignatureBuilder<M> {
        ...
        tmp.add_i64(val).expect("script size limit exceeded");
...
impl<M> ScriptSignatureBuilder<Data<M>> {
    ...
    pub fn add_data(mut self, data: &[u8]) -> ScriptSignatureBuilder<M> {
        let mut tmp = ScriptBuilder::new();
        tmp.add_data(data).expect("script size limit exceeded");
        push_reversed(&mut self.buf, &mut tmp);
        ScriptSignatureBuilder { redeem_script: self.redeem_script, buf: self.buf, _phantom: PhantomData }
    }
}
...
impl<M> ScriptSignatureBuilder<Hash<M>> {
    ...
    pub fn add_hash(mut self, hash: kaspa_hashes::Hash) -> ScriptSignatureBuilder<M> {
        let mut tmp = ScriptBuilder::new();
        tmp.add_data(&hash.as_bytes()).expect("script size limit exceeded");
        push_reversed(&mut self.buf, &mut tmp);
        ScriptSignatureBuilder { redeem_script: self.redeem_script, buf: self.buf, _phantom: PhantomData }
    }
}
...
impl<M> ScriptSignatureBuilder<R0SuccinctSeal<M>> {
    pub fn add_r0_succinct_seal(mut self, seal_words: &[u32]) -> ScriptSignatureBuilder<M> {
        let data: Vec<u8> = seal_words.iter().flat_map(|w| w.to_le_bytes()).collect();
        let mut tmp = ScriptBuilder::new();
        tmp.add_data(&data).expect("script size limit exceeded");
        push_reversed(&mut self.buf, &mut tmp);
        ScriptSignatureBuilder { redeem_script: self.redeem_script, buf: self.buf, _phantom: PhantomData }
...
impl<M> ScriptSignatureBuilder<R0SuccinctHashFn<M>> {
    pub fn add_r0_succinct_hashfn(mut self, id: R0SuccinctHashFnId) -> ScriptSignatureBuilder<M> {
        let mut tmp = ScriptBuilder::new();
        tmp.add_data(&[u8::from(id)]).expect("script size limit exceeded");
        push_reversed(&mut self.buf, &mut tmp);
        ScriptSignatureBuilder { redeem_script: self.redeem_script, buf: self.buf, _phantom: PhantomData }
    }
...
impl ScriptSignatureBuilder<()> {
    /// All missing inputs have been provided. Reverses the accumulated buffer
    /// (undoing the per-segment reversal) and appends the redeem script as a
    /// data push. Returns the complete signature script bytes.
    pub fn build(mut self) -> Vec<u8> {
        self.buf.reverse();
        let mut sb = ScriptBuilder::new();
        sb.add_data(&self.redeem_script).expect("script size limit exceeded");
        self.buf.extend_from_slice(sb.script());
        self.buf
    }
}
