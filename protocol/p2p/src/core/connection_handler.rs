...
impl ConnectionHandler {
    ...
    {
        debug!("libp2p_bridge: connecting Kaspa peer from libp2p stream with metadata {:?}", metadata);
        ...
        let channel = ServiceBuilder::new()
            .layer(MapResponseBodyLayer::new(move |body| {
                tonic::body::Body::new(CountBytesBody::new(tonic::body::Body::new(body), self.counters.bytes_rx.clone()))
            }))
            .layer(MapRequestBodyLayer::new(move |body| {
                tonic::body::Body::new(CountBytesBody::new(body, self.counters.bytes_tx.clone()))
            }))
...
impl ProtoP2p for ConnectionHandler {
    ...
    async fn message_stream(
        ...
        let (socket_address, metadata) = if let Some(info) = request.extensions().get::<MetadataConnectInfo>().cloned() {
            ...
            debug!("libp2p_bridge: Kaspa peer registered from libp2p stream; addr={socket_address}, metadata={:?}", info.metadata);
...
struct Libp2pSendRequest {
    inner: HyperSendRequest<tonic::body::Body>,
}

impl From<HyperSendRequest<tonic::body::Body>> for Libp2pSendRequest {
    fn from(inner: HyperSendRequest<tonic::body::Body>) -> Self {
        Self { inner }
    }
}

impl Service<http::Request<tonic::body::Body>> for Libp2pSendRequest {
